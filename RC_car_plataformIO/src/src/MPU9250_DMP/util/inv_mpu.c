...
#define inv_log_i     _MLPrintLog
#define inv_log_e     _MLPrintLog 
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (i2c_read(st.hw->addr, REG_6500_XA_ST_DATA, 3, regs)) {
    	if(debug)
    		inv_log_i("Reading OTP Register Error.\n");
    	return 0x07;
    }
    if(debug)
    	inv_log_i("Accel OTP:%d, %d, %d\n", regs[0], regs[1], regs[2]);
	for (i = 0; i < 3; i++) {
		if (regs[i] != 0) {
	...
	if(otp_value_zero == 0) {
		...
		if(debug) {
			inv_log_i("ACCEL:CRITERIA B\r\n");
			inv_log_i("Min MG: %7.4f\r\n", accel_st_al_min/1.f);
			inv_log_i("Max MG: %7.4f\r\n", accel_st_al_max/1.f);
		}
	...
	/* Self Test Pass/Fail Criteria C */
		...
		if(debug)
			inv_log_i("Accel:CRITERIA C: bias less than %7.4f\n", accel_offset_max/1.f);
		for (i = 0; i < 3; i++) {
			if(fabs(bias_regular[i]) > accel_offset_max) {
				if(debug)
					inv_log_i("FAILED: Accel axis:%d = %ld > 500mg\n", i, bias_regular[i]);
				result |= 1 << i;	//Error condition
			}
		}
	}
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	if(otp_value_zero == 0) {
		...
		if(debug) {
			inv_log_i("GYRO:CRITERIA B\r\n");
			inv_log_i("Max DPS: %7.4f\r\n", gyro_st_al_max/1.f);
		}
		...
		for (i = 0; i < 3; i++) {
			...
			if(st_shift_cust[i] < gyro_st_al_max) {
				if(debug)
					inv_log_i("GYRO FAIL axis:%d greater than 60dps\n", i);
				result |= 1 << i;	//Error condition
			}
		}
	}
	...
	/* Self Test Pass/Fail Criteria C */
		...
		if(debug)
			inv_log_i("Gyro:CRITERIA C: bias less than %7.4f\n", gyro_offset_max/1.f);
		for (i = 0; i < 3; i++) {
			if(fabs(bias_regular[i]) > gyro_offset_max) {
				if(debug)
					inv_log_i("FAILED: Gyro axis:%d = %ld > 20dps\n", i, bias_regular[i]);
				result |= 1 << i;	//Error condition
			}
		}
	}
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    if(debug)
    	inv_log_i("Starting Bias Loop Reads\n");
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    if(debug)
    	inv_log_i("Starting MPU6500 HWST!\r\n");
    ...
    if (ii == tries) {
         ...
         */
        if(debug)
        	inv_log_i("Retrieving Biases Error - possible I2C error\n");
    ...
    if(debug)
    	inv_log_i("Retrieving ST Biases\n");
    ...
    if (ii == tries) {
        ...
        if(debug)
        	inv_log_i("Retrieving ST Biases Error - possible I2C error\n");
    ...
    if(debug)
    	inv_log_i("Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if(debug)
    	inv_log_i("Gyro Self Test Results: %d\n", gyro_result);
    ...
    if(debug)
    	inv_log_i("Compass Self Test Results: %d\n", compass_result);
    if (!compass_result)
        result |= 0x04;
#else
    result |= 0x04;
