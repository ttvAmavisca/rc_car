/*
 $License:
    ...
    See included LicenFVse.txt for License information.
 $
 */
/**
...
#ifdef AK89xx_SECONDARY
        mpu_set_compass_sample_rate(__min(st.chip_cfg.compass_sample_rate, MAX_COMPASS_SAMPLE_RATE));
#endif
...
#define LOAD_CHUNK  (16)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        this_write = __min(LOAD_CHUNK, length - ii);
...
{  
 ...
 // reset device
  data_w[0]= 0x80;
  i2c_write(st.hw->addr, st.reg->pwr_mgmt_1,1,  data_w); // Write a one to bit 7 reset bit; toggle reset device
  delay(100);
   
 // get stable time source; Auto select clock source to be PLL gyroscope reference if ready 
 // else use the internal oscillator, bits 2:0 = 001
  data_w[0]=0x01;
  i2c_write(st.hw->addr, st.reg->pwr_mgmt_1,1, data_w);  
  data_w[0]= 0x00;
  i2c_write(st.hw->addr, st.reg->pwr_mgmt_2,1, data_w);
  delay(200);                                    

// Configure device for bias calculation
  data_w[0]= 0x00;
  i2c_write(st.hw->addr, st.reg->int_enable,1, data_w);   // Disable all interrupts
  i2c_write(st.hw->addr, st.reg->fifo_en,1, data_w);      // Disable FIFO
  i2c_write(st.hw->addr, st.reg->pwr_mgmt_1,1, data_w);   // Turn on internal clock source
  i2c_write(st.hw->addr, st.reg->i2c_mst,1, data_w); // Disable I2C master
  i2c_write(st.hw->addr, st.reg->user_ctrl,1, data_w);    // Disable FIFO and I2C master modes
  data_w[0]= 0x0C;
  i2c_write(st.hw->addr, st.reg->user_ctrl,1, data_w);    // Reset FIFO and DMP
  delay(15);
  
// Configure MPU6050 gyro and accelerometer for bias calculation
  data_w[0]= 0x01;
  i2c_write(st.hw->addr, st.reg->lpf,1, data_w);      // Set low-pass filter to 188 Hz
  data_w[0]= 0x00;
  i2c_write(st.hw->addr,st.reg->rate_div,1, data_w);  // Set sample rate to 1 kHz
  i2c_write(st.hw->addr, st.reg->gyro_cfg ,1,data_w);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
  i2c_write(st.hw->addr, st.reg->accel_cfg,1, data_w); // Set accelerometer full-scale to 2 g, maximum sensitivity
...
// Configure FIFO to capture accelerometer and gyro data for bias calculation
  data_w[0]= 0x40;
  i2c_write(st.hw->addr, st.reg->user_ctrl,1, data_w);   // Enable FIFO  
  data_w[0]= 0x78;
  i2c_write(st.hw->addr, st.reg->fifo_en,1, data_w);     // Enable gyro and accelerometer sensors for FIFO  (max size 512 bytes in MPU-9150)
  delay(40); // accumulate 40 samples in 40 milliseconds = 480 bytes

// At end of sample accumulation, turn off FIFO sensor read
  data_w[0]= 0x00; 
  i2c_write(st.hw->addr, st.reg->fifo_en,1, data_w);        // Disable gyro and accelerometer sensors for FIFO
  i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, &data[0]); // read FIFO sample count
  fifo_count = ((uint16_t)data[0] << 8) | data[1];
  packet_count = fifo_count/12;// How many sets of full gyro and accelerometer data for averaging
  
  for (ii = 0; ii < packet_count; ii++) {
    int16_t accel_temp[3] = {0, 0, 0}, gyro_temp[3] = {0, 0, 0};
    i2c_read(st.hw->addr, st.reg->fifo_r_w , 12, &data[0]); // read data for averaging
...
// Push gyro biases to hardware registers
  i2c_write(st.hw->addr,  0x13,1, &data[0]);
  i2c_write(st.hw->addr, 0x14,1, &data[1]);
  i2c_write(st.hw->addr, 0x15,1, &data[2]);
  i2c_write(st.hw->addr, 0x16,1, &data[3]);
  i2c_write(st.hw->addr, 0x17,1, &data[4]);
  i2c_write(st.hw->addr, 0x18,1, &data[5]);
...
// the accelerometer biases calculated above must be divided by 8.
  ...
  i2c_read(st.hw->addr, st.reg->accel_offs, 2, &data[0]); // Read factory accelerometer trim values
  accel_bias_reg[0] = (int32_t) (((int16_t)data[0] << 8) | data[1]);
  i2c_read(st.hw->addr, 0x7A, 2, &data[0]);
  accel_bias_reg[1] = (int32_t) (((int16_t)data[0] << 8) | data[1]);
  i2c_read(st.hw->addr, 0x7D, 2, &data[0]);
