...
*/
/**************************************************************************/
float BMP280::readPressure(bool updateTMP) {
  int64_t var1, var2, p;

  // Must be done first to get the t_fine variable set up
  if (updateTMP)readTemperature();
...
bool BMP280::actualizar()
{
   if (millis() > _time_last_update){

     last_temperature=readTemperature();
     last_presure=readPressure(false);
   
    
    _time_last_update =millis() +500; //cambiar con standby
    return true;
  }
  return false;
}
